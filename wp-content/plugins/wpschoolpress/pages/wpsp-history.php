...
if( is_user_logged_in() ) {
  ...
  $stinfo  =  $wpdb->get_row("select CONCAT_WS(' ', s_fname, s_mname, s_lname ) AS full_name from $student_table where wp_usr_id='$student_id'");
    ...
    <div class="wpsp-card">
      <div class="wpsp-card-body">
        <?php
        if ($current_user_role == 'administrator' || $current_user_role == 'teacher') {
          echo "<h3 class='wpsp-card-title'>".$stinfo->full_name."'s  history</h3>";
        }
        ?>
          ...
          <tbody>
            ...
            <?php
              ...
              $historyData  =  $wpdb->get_results("SELECT ct.cid, ct.c_name, st.wp_usr_id, CONCAT_WS(' ', st.s_fname, st.s_mname, st.s_lname ) AS full_name, ht.* , CONCAT_WS(' ', tt.first_name, tt.middle_name, tt.last_name ) AS t_full_name FROM $history_table ht
                ...
                JOIN $student_table st ON ht.s_id = st.wp_usr_id WHERE st.wp_usr_id = $student_id");
                if(empty($historyData)){
                  echo "<tr><td colspan='6' align='center'><strong>History Not Found!</strong></td></tr>";
                }else{
              foreach ($historyData as $history) {
                $student_count = $wpdb->get_results("SELECT class_id FROM $student_table WHERE wp_usr_id = $student_id ");
                $scount = 0;
                ...
                echo "<tr>
                <td>".$history->h_id."</td>
                <td>".$history->c_name."</td>
                <td>".$history->t_full_name."</td>
                <td>".$history->start_date."</td>
                <td>".$history->end_date."</td>
                <td>".date('Y-m-d',strtotime($history->enrollment_date))."</td>
                </tr>";
              }
            }
              ?>
