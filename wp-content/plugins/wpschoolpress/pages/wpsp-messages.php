...
if (is_user_logged_in()) {
  ...
  if ($current_user_role == 'administrator' || $current_user_role == 'teacher' || $current_user_role == 'parent' || $current_user_role = 'student') {
    ...
    if (isset($_GET['mid'])){
    $sender_id = $wpdb->get_results("select s_id from $messages_table where mid = $mid");
    ...
    if($sender_id[0]->s_id == $current_user->ID){
      ...
      $unread_msg_count = $wpdb->get_results("select count(*) from $messages_table where main_m_id= $mid AND r_read = 0 AND r_id = $current_user->ID");
                ...
                <div class="mailbox-header">
                  <div class="wpsp-card">
                    <div class="wpsp-card-head">
                      <button id="createMessage" class="wpsp-btn btn btn-primary btn-block" data-pop="ViewModal"  data-toggle="modal" data-target="#newMessage"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Compose </button>
                    </div>
                    <div class="wpsp-card-body">
                      <ul class="list-unstyled mailbox-nav">
                        <li class="active">
                          <a href="<?php echo wpsp_admin_url();?>sch-messages&tab=inbox">
                            <i class="fa fa-inbox"></i> Inbox<span class="badge badge-success pull-right btn-primary"><?php echo wpsp_UnreadCount(); ?></span>
                          </a>
                        </li>

                        <li><a href="<?php echo wpsp_admin_url(); ?>sch-messages&tab=sentbox">
                          <i class="fa fa-sign-out"></i>Sent</a>
                        </li>
                        <li>
                          <a href="<?php echo wpsp_admin_url(); ?>sch-messages&tab=trash">
                      ...
                      <div class="wpsp-col-md-12 table-responsive">
                        ...
                        if (isset($_GET['mid']) && !empty(sanitize_text_field($_GET['mid']))){
                          ...
                          if (isset($_REQUEST['tab']) && sanitize_text_field($_REQUEST['tab']) == 'inbox'){
                            ...
                            <div class="wpsp-row">
                              <div class="wpsp-col-md-12">
                                <h3 style="margin-top:0;margin-bottom:5px;font-weight: 500;">Leave a Reply</h3>
                        ...
                        <table class="table table-bordered wpsp-table" id="message-list">
                          <thead>
                            <tr>
                              ...
                              <th>Subject</th>
                              <th>Date</th>
                              <th class="nosort" style="max-width:100px;">Action</th>
                            </tr>
                          </thead>
                          <tbody>
                            <?php
                            if ($currentTab == 'trash') {
                              $mid = $wpdb->get_results("select distinct main_m_id from $messages_table where ( r_id=$current_user->ID or s_id=$current_user->ID ) and main_m_id IN (SELECT m_id FROM $messages_delete_table WHERE user_id = $current_user->ID AND delete_status = 0 )  order by mid DESC");

                            } else if ($currentTab == 'inbox') {

                              $mid = $wpdb->get_results("select distinct main_m_id from $messages_table where ( r_id=$current_user->ID ) and main_m_id NOT IN (SELECT distinct m_id FROM $messages_delete_table WHERE user_id = $current_user->ID ) order by mid DESC");


                            } else {

                              $mid = $wpdb->get_results("select distinct main_m_id from $messages_table where ( s_id=$current_user->ID ) and main_m_id NOT IN (SELECT distinct m_id FROM $messages_delete_table WHERE user_id = $current_user->ID ) order by mid DESC");
                            }
                            if(!empty($mid)){
                              foreach ($mid as $id) {

                                $sub_msg_count = 0;
                                $read_mcount_q = $wpdb->get_results("select mid from $messages_table where main_m_id= $id->main_m_id AND r_read = 0 AND r_id = $current_user->ID");
                                ...
                                if (!isset($_REQUEST['tab']) OR sanitize_text_field($_REQUEST['tab']) == 'inbox'){
                                  if($sender_id[0]->s_id == $current_user->ID){
                                    $read_mcount = $wpdb->get_var("select count(*) from $messages_table where main_m_id= $id->main_m_id AND s_read = 0 AND r_id = $current_user->ID");
                                  }else{
                                    $read_mcount = $wpdb->get_var("select count(*) from $messages_table where main_m_id= $id->main_m_id AND r_read = 0 AND r_id = $current_user->ID");
                                  }
                                }else{
                                  $read_mcount = 0;
                                }
                                ...
                                if($id->main_m_id != 0){
                                  if ($currentTab == 'trash') {
                                    $msgb = $wpdb->get_results("select * from $messages_table where main_m_id = $id->main_m_id");
                                  }else{
                                    $msgb = $wpdb->get_results("select * from $messages_table where (del_stat IS NULL OR del_stat != $current_user->ID) and main_m_id = $id->main_m_id");
                                  }
                                }else{
                                  $msgb = $wpdb->get_results("select * from $messages_table where ( r_id = $current_user->ID ) and de(del_stat IS NULL OR del_stat != $current_user->ID) and main_m_id = $id->main_m_id");
                                }

                                if(!empty($msgb)){
                                  $msg_id  = $msgb[0]->main_m_id;
                                  if (isset($_REQUEST['tab']) && sanitize_text_field($_REQUEST['tab']) == 'inbox'){
                                  ...
                                  if ($display) {
                                    ?>
                                    <tr class="<?php echo (($read_mcount != 0)? "unread": "read");?>">
                                      <td><input type="checkbox" class="mid_checkbox ccheckbox" name="mid[]" value="<?php echo intval($msgb[0]->mid); ?>" /></td>
                                      <td class="name"><?php echo $nickname; ?> (<span class="label label-role-msg label-role-<?php echo $role; ?>"><?php echo $role;?></span>)</td>
                                      <td class="subject"><?php echo (($sub_msg_count != 0)? "<strong>(".$sub_msg_count.")</strong> " : ""); ?><?php echo $msg_text; ?></td>
                                      <td class="time"><?php echo wpsp_ViewDate($msgb[0]->m_date); ?></td>
                                      <td class="small-col">
                                        <?php
                                        if (sanitize_text_field($_REQUEST['tab']) != 'trash') {
                                        ?>
                                        <a class="pointer text-blue viewMess" title="view" href="<?php echo wpsp_admin_url(); ?>sch-messages&tab=<?php echo $currentTab;?>&mid=<?php echo intval($msgb[0]->mid); ?>">
                                      ...
                                      <?php } ?>
                                        <a href="javascript:;" title="Delete"  class="delete_messages" <?php
                                         ...
                                         ?> data-id="<?php echo intval($msgb[0]->main_m_id); ?>"  mid="<?php echo intval($msgb[0]->main_m_id);?>">
          ...
          <form class="form-horizontal group-border-dashed" action="" id="newMessageForm" style="border-radius: 0px;" method="post">
             ...
             if ($current_user_role == 'administrator'){
              ...
              $students = array_unique($studentData);
               ...
               $student_list = $wpdb->get_results("SELECT wp_usr_id from $studenttable WHERE sid in ( select DISTINCT st.sid from $studenttable st WHERE st.parent_wp_usr_id = $parent_id) ");
            ...
            if ($current_user_role == 'administrator'){
              ...
              $s_data = $wpdb->get_results("SELECT class_id FROM $studenttable where parent_wp_usr_id = $parent_id");
              $class_data = [];
              ...
              $s_data = $wpdb->get_results("SELECT class_id FROM $studenttable where wp_usr_id = $student_id");
            ...
            <div class="wpsp-form-group">
              <div class="wpsp-row">
                <label class="wpsp-col-sm-3 control-label">Receiver</label>
                <div class="wpsp-col-sm-9">
                  <ul class="wpsp-receiver-links">
                    <li class="active">
                      <a href="javascript:;" id="showGroup">Select Group</a>
                    </li>
                    <?php if(!empty($teachers)) { ?>
                    <li>
                      <a href="javascript:;" id="showTeachers"><?php echo _t( 'Select Teachers' ); ?></a>
                    </li>
                  <?php }
                  if(!empty($students)){
            ...
            <div class="wpsp-form-group" id="receiverTeachers">
              <div class="wpsp-row">
                <label class="wpsp-col-sm-3 control-label"><?php echo _t( 'Select Teacher' ); ?></label>
                <div class="wpsp-col-sm-9" >

                  <select name="r_id[]" multiple class="r_id" class="wpsp-form-control teacher_multi_select">
                    <?php
                      if (!empty($teachers)) {
                        echo '<optgroup label="' . _t( 'All Teachers' ) . '">';
                        foreach ($teachers as $id) {
                          ...
                          ?>
                          <option value="<?php echo intval($teachers_data[0]->wp_usr_id); ?>"><?php echo $teachers_data[0]->full_name; ?></option>
                          <?php
                        }
                        echo '</optgroup>';
                      }
                ...
                <div class="wpsp-form-group" id="receiverStudents">
                        ...
                        <?php
                          ...
                          if (!empty($students)) {
                            ...
                            foreach ($students as $sinfo) {
                              ...
                              ?>
                                <option value="<?php echo intval($sinfo); ?>"><?php echo $student_list[0]->full_name; ?></option>
                              <?php
                  ...
                  <div class="wpsp-form-group" id="receiverParents">
                    <div class="wpsp-row">
                      <label class="wpsp-col-sm-3 control-label">Select Parents</label>
                      <div class="wpsp-col-sm-9" >
                            ...
                            if (!empty($parents)) {
                              ...
                              foreach ($parents as $key => $pinfo) {
                                ...
                                if(intval(intval($parent_list[0]->parent_wp_usr_id)) != 0 ){
                                  ...
                                  ?>
                                  <option value="<?php echo intval($parent_list[0]->parent_wp_usr_id); ?>"><?php echo $parent_name; ?></option>
                                  <?php
                                }
                              }
                              echo '</optgroup>';
                ...
                <div class="wpsp-form-group" id="receiverGroups">
                  <div class="wpsp-row">
                    <label class="wpsp-col-sm-3 control-label">Select Group</label>
                    <div class="wpsp-col-lg-9 wpsp-col-md-9 none" >
                      ...
                      if(!empty($teachers)){
                        ...
                        <optgroup label="<?php echo _t( 'Class Teachers' ); ?>">
                          <?php
                          $class_table = $wpdb->prefix . "wpsp_class";
                          $class_ids   = $wpdb->get_results("select cid,c_name from $class_table");
                          foreach ($class_ids as $clss) {
                            ?>
                            <option value="t.<?php echo intval($clss->cid); ?>"> Class <?php echo $clss->c_name; ?> <?php echo _t( 'Teachers' ); ?> </option>>
                            <?php
                          }
                          ?>
                        </optgroup>
                        ...
                        <optgroup label="Class Parents">
                          ...
                          foreach ($class_ids as $clss) {
                            ?>
                            <option value="p.<?php echo intval($clss->cid); ?>"> Class <?php echo $clss->c_name; ?> Parents </option>
                            <?php
                          }
                          ?>
                        </optgroup>
                ...
                <div class="wpsp-form-group">
                  <div class="wpsp-row">
                    <label class="wpsp-col-sm-3 control-label">Subject</label>
                      <div class="wpsp-col-sm-9">
                ...
                <div class="wpsp-form-group">
                  <div class="wpsp-row">
                    <label class="wpsp-col-sm-3 control-label">Message</label>
                      <div class="wpsp-col-sm-9">
