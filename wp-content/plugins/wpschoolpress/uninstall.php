...
if ( !defined( 'ABSPATH' ) ) {	exit; }
	...
	if( $remove_data_status== 1 ) {		
		...
		$page_titles=array( 'Teacher','Student','Class','Dashboard','Messages','Exams','Attendance','Timetable','Events','Subject','Transport','Settings','Marks','Notify','SMS','ImportHistory','LeaveCalendar', _t( 'TeacherAttendance' ), 'Parent', 'Change Password', 'Payment' );		
...
if ( !defined( 'ABSPATH' ) ) {	exit; }
	...
	if( $remove_data_status== 1 ) {		
		...
		$wpsp_fees_payment			=	$wpdb->prefix.'wpsp_fees_payment';		
		$wpsp_fee_payment_history	=	$wpdb->prefix.'wpsp_fee_payment_history';		
		$page_titles=array( 'Teacher','Student','Class','Dashboard','Messages','Exams','Attendance','Timetable','Events','Subject','Transport','Settings','Marks','Notify','SMS','ImportHistory','LeaveCalendar', _t( 'TeacherAttendance' ), 'Parent' );		
		foreach($page_titles as $page_title) {			
			$page = get_page_by_title($page_title);			
			wp_delete_post($page->ID,true);	//Trash or delete a post or page.	
		}		
